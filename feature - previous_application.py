...
@Package 
@author: ly
@date Date: 2019年04月11日 14:52
@Description: 
@URL: https://github.com/oskird/Kaggle-Home-Credit-Default-Risk-Solution/blob/master/Feature/feature%20-%20previous_application.py
@version: V1.0
'''
import pandas as pd
...
#把HOUR_APPR_PROCESS_START、NFLAG_LAST_APPL_IN_DAY、NFLAG_INSURED_ON_APPROVAL
#属性也改为object
cat_pa = pd.concat( [previous_application[['HOUR_APPR_PROCESS_START', \
                                           'NFLAG_LAST_APPL_IN_DAY', \
                                           'NFLAG_INSURED_ON_APPROVAL']], cat_pa], axis=1 ).fillna( 'XNA' )
...
for i in range( -5, 0 ):
    num_pa.iloc[ :, i ] = num_pa.iloc[ :, i ].map( lambda x:np.nan if x==36243.0 else x )
...
del pa_num, pa_cat
gc.collect()
pa_feature.to_csv(  'pa_featur.csv' )
